/*

Objekt udrzuje az 4 spojeni seriove porty - singleton
public funkce pro odeslilani bytearray
listenery pro prijem zpravy - kontrola, jestli je validni, crc atd...



 */
package SerialCom;

import Configuration.ConfXmlObject;
import Configuration.ConfXmlObject.Device;
import Configuration.Configuration;
import Devices.DeviceInterface;
import java.time.Instant;
import java.util.ArrayList;
import java.util.List;
import java.util.concurrent.TimeUnit;
import java.util.concurrent.locks.Condition;
import java.util.concurrent.locks.Lock;
import java.util.concurrent.locks.ReentrantLock;
import java.util.function.Predicate;
import java.util.logging.Level;
import java.util.logging.Logger;
import java.util.stream.Collectors;

/**
 *
 * @author husak
 */
public final class SerialCommunication {

    private static SerialCommunication singletonObj = null;
    private List<ConnectionThread> connections = null;
    private List<ConfXmlObject.Device> devicesList = null;
    //private SerialProtocol protocol = null;
    private final int poolInterval = 100; // 1 device cca 30 ms
    private boolean communicationActive = true;
    private SerialUpdater updater;

    public static synchronized SerialCommunication getInstance() {
        if (singletonObj == null) {
            singletonObj = new SerialCommunication();
        }

        return singletonObj;
    }

    private SerialCommunication() {
        System.out.println("SerialCommunication start");

        //protocol = new SerialProtocol();
        connections = new ArrayList<>();
        updater = new SerialUpdater();

        initConnetions();
        startDataPooling();
    }
    
    private void initConnetions()
    {
        devicesList = Configuration.getXMLObj().getDevices();

        // load from config
        for (ConfXmlObject.Connection c : Configuration.getXMLObj().getConnections()) {
            ConnectionThread t = new ConnectionThread(c.name, c.portName, 115200);
            t.start();
            connections.add(t);

            System.out.println("Connection: " + c.name + ", " + c.portName);
        }
    }
              

    public synchronized void reloadConfiguration() {
        System.out.println("Reloading SerialCommunication configuration...");

        try {
            // Zastavit komunikaci
            stopDataPooling();
            Thread.sleep(500);

            // Odpojit všechna existující připojení
            for (ConnectionThread conn : connections) {
                conn.connectionDisconnect();
            }

            // Vyčistit seznam
            connections.clear();

            initConnetions();

            // Spustit komunikaci znovu
            Thread.sleep(500);
            startDataPooling();

            System.out.println("SerialCommunication configuration reloaded successfully");

        } catch (Exception e) {
            System.err.println("Error reloading SerialCommunication: " + e.getMessage());
            e.printStackTrace();
        }
    }

    public synchronized void startDataPooling() {
        communicationActive = true;
    }

    public synchronized void stopDataPooling() {
        communicationActive = false;
    }

    public synchronized void sendAsyncMsg(String connName, String msg) {
        for (ConnectionThread conn : connections) {
            if (conn.connectionName.equals(connName)) {
                conn.connectionDataSend(msg);
            }
        }
    }

    public void sendCommand(String connection, String message) {
        Thread CommandWorker = new Thread() {
            @Override
            public void run() {
                SerialCommunication.getInstance().stopDataPooling();
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    System.err.println(e);
                }
                sendAsyncMsg(connection, message);
                SerialCommunication.getInstance().startDataPooling();
            }
        };
        CommandWorker.start();
    }

    private final class ConnectionThread extends Thread {

        private List<String> devicesName;
        public final String connectionName;
        private final String portName;
        private final int baudRate;
        private final SerialConnection serialCon;
        //private boolean communicationActive = false;

        // synchronization serial 
        private final Lock lock;
        private final Condition msgReceived;
        private long begin;
        private long end;
        private long maxDuration = 0;
        private long lastResponse;

        ConnectionThread(String connectionName, String portName, int baudRate) {
            Thread.currentThread().setName("ConnectionThread " + portName);

            devicesName = new ArrayList<>();
            this.connectionName = connectionName;
            this.portName = portName;
            this.baudRate = baudRate;

            lock = new ReentrantLock();
            msgReceived = lock.newCondition();

            serialCon = new SerialConnection() {
                @Override
                public void received(String data) {
                    connectionDataReceived(data);
                }
            };

            System.out.println(devicesList);
            // get device names
            for (Device d : devicesList) {
                if (d.connection.equals(connectionName)) {
                    devicesName.add(d.name);
                }

            }
            connectionConnect();
        }

        public String getPortName() {
            return portName;
        }

        private void poolAllDevices() {
            for (String device : devicesName) {

                connectionDataSend(DeviceInterface.getInstance().getSendSerialData(device));

                // wait for reply
                lock.lock();
                try {
                    boolean code = msgReceived.await(100, TimeUnit.MILLISECONDS);
                    if (code == false) {
                        //System.err.println(java.time.LocalDateTime.now() + " Device not responding:" + device);
                    }
                } catch (InterruptedException e) {
                    System.err.println(e);
                } finally {
                    lock.unlock();
                }

                // sleep
                try {
                    Thread.sleep(15);
                } catch (InterruptedException ex) {
                    System.err.println(ex);
                }
            }
        }

        private void connectionDataReceived(String data) {

            System.out.println(java.time.LocalDateTime.now() + " RX " + portName + ":" + data);
            //protocol.protocolParseReceivedData(connectionName, data);
            DeviceInterface.getInstance().parseIncomingSerialData(data);
            updater.parseSerialMsg(portName, data);
            
            lastResponse = Instant.now().toEpochMilli();

            try {
                //P040_ServicePageController.getInstance().addMessage(connectionName, data);
            } catch (NullPointerException e) {
                System.err.println(e);
            }

            lock.lock();
            try {
                msgReceived.signal();
            } finally {
                lock.unlock();
            }
        }

        private void connectionDataSend(String data) {

            //System.out.println(java.time.LocalDateTime.now() + " TX " + portName + ":" + data);
            try {
                //P040_ServicePageController.getInstance().addMessage(connectionName, data);
            } catch (NullPointerException e) {
                //System.err.println(e);
            }
            serialCon.send(data);

        }

        public void connectionConnect() {
            serialCon.connect(portName, baudRate);
        }

        public void connectionDisconnect() {
            serialCon.disconnect();
        }

        @Override
        public void run() {

            try {
                Thread.sleep(2000);
            } catch (InterruptedException ex) {
                System.err.println(ex);
            }

            while (true) {
                begin = Instant.now().toEpochMilli();

                if (communicationActive) {
                    poolAllDevices();
                    //ControlEngine.ControlEngine.getInstance().allDevicesPoolled(connectionName);

                    // check if is response - reconect
                    if ((lastResponse + 3000) < Instant.now().toEpochMilli()) {
                        connectionDisconnect();
                        connectionConnect();
                        System.out.println(java.time.LocalDateTime.now() + " No response from devices - reconnect connection " + portName + " " + connectionName);
                    }
                }

                end = Instant.now().toEpochMilli();

                long duration = end - begin;

                if (duration > maxDuration) {
                    maxDuration = duration;
                    //System.out.println(java.time.LocalDateTime.now() + ": " + duration);
                }

                //System.out.println(java.time.LocalDateTime.now() + ": " + duration);
                if (duration < poolInterval) {
                    try {
                        if ((poolInterval - duration) > 0) {
                            Thread.sleep(poolInterval - duration);
                        }
                    } catch (InterruptedException ex) {
                        System.err.println(ex);
                    }
                }

                //System.out.println("comm cycle");
            }
        }
    }

    public SerialUpdater getSerialUpdater() {
        return updater;
    }

    private final char crc8_table[] = {
        };

    public char crc8(int seed, char[] data, int len) {
        char crc = (char) seed;

        if (data == null) {
            return 0;
        }

        crc &= 0xff;

        for (int i = 0; i < len; i++) {
            crc = crc8_table[crc ^ data[i]];
        }
        return crc;
    }
}
